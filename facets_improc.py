...
def imread(fp):
    with PIL.Image.open(fp, "r") as h:
        img = np.array(h, dtype=np.int64)
    return img


def imread_tar(tar_name, member):
    with tarfile.open(tar_name, mode="r") as h:
        img = imread(h.extractfile(member))
    return img
...
def imshow_reduced_color_depth(img_a, d, new_figure=True):
    if d < 0 or d > 7:
